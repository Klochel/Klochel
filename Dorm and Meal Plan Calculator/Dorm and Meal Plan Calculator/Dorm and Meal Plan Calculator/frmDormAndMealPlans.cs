...
namespace Dorm_and_Meal_Plan_Calculator
{
    ...
    public partial class frmDormAndMealPlans : Form
    {
        ...
        private void btnCALCULATE_Click(object sender, EventArgs e)
        {
            ...
            if (cbxAllenHall.Checked)
            {
                value1 += (double)Dorm.allen;
                total += value1;
            }
            else if (cbxPikeHall.Checked)
            {
                value1 += (double)Dorm.pike;
                total += value1;
            }
            else if (cbxFarthingHall.Checked)
            {
                value1 += (double)Dorm.farthing;
                total += value1;
            }
            else if (cbxUniversitySuites.Checked)
            {
            ...
            if (cbx7MealsPerWeek.Checked)
            {
                value2 += (double)Dorm.sevenMeals;
                total = value1 + value2;
            }
            else if (cbx14MealsPerWeek.Checked)
            {
                value2 += (double)Dorm.fourteenMeals;
                total = value1 + value2;
            }
            else if (cbxUnlimitedMeals.Checked)
            {
            ...
            totalCharges.lblTotalCharges = total.ToString();
